...
defaults to 1 second. A failed write or flush causes the plugin to reopen the
pipe and send a new hello followed by a fresh snapshot. The polling loop is
deliberately conservative and must be replaced by event-driven publication or
an accepted performance budget before production use.
