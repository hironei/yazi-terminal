...
still be processing it. The launcher shows a warning before exiting when
delivery cannot be confirmed. The option is current-user-only and does not
reuse a Git Bash or other terminal pane.
...
protection. If a right-click becomes stale between button-down and release,
